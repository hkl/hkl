...
HklValue* hklr_op_mod(HklValue* left_value, HklValue* right_value)
{
  ...
  switch (left_value->type)
  {
      ...
      switch(right_value->type)
      {
        ...
        case HKL_TYPE_REAL:
          result = hkl_value_new(HKL_TYPE_REAL,
            left_value->as.integer % (int)right_value->as.real);
          break;
    ...
    case HKL_TYPE_REAL:
      switch(right_value->type)
      {
        case HKL_TYPE_INT:
          result = hkl_value_new(HKL_TYPE_REAL,
            (int) left_value->as.real % right_value->as.integer);
          break;

        case HKL_TYPE_REAL:
          result = hkl_value_new(HKL_TYPE_REAL,
            (int) left_value->as.real % (int) right_value->as.real);
          break;
        default:
          assert(false);
          break;
